...
#         if self.training:
#             # 💡 [핵심 패치] 최신 Ultralytics Loss(v8DetectionLoss) 함수 규격에 맞게 
#             # 딕셔너리 대신 (결합된_텐서, 피처맵_리스트) 형태의 튜플로 반환합니다.
            
#             # 1. Loss 엔진이 그리드를 그릴 수 있도록 공간 해상도가 유지된 피처맵 리스트 조립
#             feats = [torch.cat((box_outs[i], cls_outs[i]), 1) for i in range(self.nl)]
            
#             # 2. B, C, Anchors 형태로 모든 스케일 플래튼 및 병합 (y)
#             # self.no = nc + self.reg_max * 4 (예: 8 + 64 = 72)
#             y = torch.cat([xi.view(shape[0], self.no, -1) for xi in feats], 2)
            
#             # 3. 순정 YOLOv8 Detect 헤드와 완벽히 동일한 반환 규격
#             return (y, feats)
...
#             for i in range(self.nl):
#                 b, _, h, w = x[i].shape
#                 xi = torch.cat((box_outs[i], cls_outs[i]), 1)
#                 # 텐서 플래튼 (Flatten)
#                 xi = x[i].view(b, self.no, h * w)
                
#                 # Box 디코딩 (DFL 적용)
#                 box, cls_score = xi.split((self.reg_max * 4, self.nc), 1)
#                 box = self.dfl(box)
                
#                 # Sigmoid를 통한 Confidence 추출
#                 cls_score = cls_score.sigmoid()
#                 y.append(torch.cat((box, cls_score), 1))
